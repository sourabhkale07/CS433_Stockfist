...
Position& Position::setCS433(const string& fenStr, bool isChess960, StateInfo* si) {
...
bool Position::isCheck(){
    const Stockfish::Color white = Stockfish::WHITE, black = Stockfish::BLACK;

    // Update slider blockers and pinners for both sides
    ...
    std::cout<<st->blockersForKing[white]<<std::endl;
    std::cout<<st->blockersForKing[black]<<std::endl;

    // Check if the white king is in check
    Stockfish::Square wKingSquare = square<Stockfish::KING>(white);
    Stockfish::Bitboard attackers = attackers_to(wKingSquare) & pieces(black);
    attackers &= ~st->blockersForKing[white]; // Remove blockers for the white king
    if (attackers != 0) {
        // White king is in check, check for pinned pieces
        Stockfish::Bitboard pinners = st->pinners[black] & pieces(white);
        if (pinners == 0) {
            // No pinned pieces, white king is in check
            return true;
        }
    }

    // Check if the black king is in check
    Stockfish::Square bKingSquare = square<Stockfish::KING>(black);
    attackers = attackers_to(bKingSquare) & pieces(white);
    attackers &= ~st->blockersForKing[black]; // Remove blockers for the black king
    if (attackers != 0) {
        // Black king is in check, check for pinned pieces
        Stockfish::Bitboard pinners = st->pinners[white] & pieces(black);
        if (pinners == 0) {
            // No pinned pieces, black king is in check
            return true;
        }
    }
